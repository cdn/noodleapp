...
var createClientMessage = function(options, callback) {
  process.nextTick(function() {
    ...
    if (options.recent.reply_to || options.recent.num_replies) {
      isThread = true;
    }
    ...
    if (recentMessages.length > 0) {
      for (var i = 0; i < recentMessages.length; i ++) {
        ...
        if (recent.text) {
          ...
          if (recent.repost_of)
            recent.annotations = recent.repost_of.annotations;

          if (recent.annotations) {
            recent.annotations.forEach(function(annotation) {
              if (annotation.type === 'net.xtendr.mood' && moodArray.indexOf(annotation.value.mood) > -1) {
                ...
                if (annotation.value.type === 'photo' &&
                  recent.text.indexOf(annotation.value.url) === -1) {
...
/*      remix.result = media.text + ' <div class="object-wrapper"><iframe width="' + options.width + '" height="' +
	...
	http://api.bli.ms/oembed/iframe/19275/
	http://api.bli.ms/oembed/iframe/' + annotation.value.short_code + '/
*/
                if (annotation.value.type === 'video' &&
                  recent.text.indexOf(annotation.value.url) === -1) {
                  var linkUrl = annotation.value.embeddable_url || annotation.value.url;
                  var photo = '<div class="object-wrapper">';
                  photo += annotation.value.html;
//                  photo += '<iframe width="' + options.width + '" height="' +
//        options.height + '" src="//www.youtube.com/embed/' + youtubeId +
...
//            } else if (annotation.type === 'net.app.core.crosspost' && recent.text.indexOf(annotation.value.url) === -1)) {
              } else if (annotation.type === 'net.app.core.crosspost') {
                var crosspostUrl = annotation.value.canonical_url;
                crosspost = crosspostUrl
              } else if (annotation.type === 'net.pastapp.mots') {
                recent.text = annotation.value.text;
              }
            });
          }
